...
//     canvasAttn.style.width = numPatches * ATTN_SCALING + "px";
//     canvasAttn.style.height = numPatches * ATTN_SCALING + "px";
//     matrixColorsAttn = Array(numPatches).fill().map(() => Array(numPatches).fill(''));
//     for (let i = 0; i < numPatches; i++) {
//         for (let j = 0; j < numPatches; j++) {
//             var color = getColor(attnHead[currentAttnIndex][i][j]);
//             ctxAttn.fillStyle = color;
...
//     patchSize = Math.floor(imageSize / Math.sqrt(numPatches - clsTokenOffset));
//     numPatchWidth = Math.ceil(imageSize / patchSize);
...
var patchSize;
var numPatchWidth;
var imageScaleFactor = 4; // scale factor to increase image size by four times
...
function setNewImage(currentImageIndex) {
    var originalImageSize = imageSizes[currentImageIndex];
    var imageSize = originalImageSize * imageScaleFactor;

    canvasImg.width = imageSize;
    canvasImg.height = imageSize;
    canvasImg.style.width = imageSize + "px";
    canvasImg.style.height = imageSize + "px";
